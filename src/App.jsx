...
export default function App() {
  ...
  const canUseProtectedApi = useMemo(() => baseUrl.trim() && token.trim(), [baseUrl, token]);

  const testConnection = async () => {
    setLoading((prev) => ({ ...prev, health: true }));
    try {
      const data = await apiClient.getHealth(baseUrl);
  ...
  return (
    <div className="container">
      <header>
        ...
        <p>Playwright 실행은 서버 API로 위임되고, 이 앱은 정적 React 프론트엔드로 동작합니다.</p>
      </header>
      ...
      <div className="layout">
        ...
        <JobDetail detail={jobDetail} baseUrl={baseUrl} token={token} artifactUrlBuilder={apiClient.artifactUrl} />
      </div>
      ...
      <footer>
        ...
        <button type="button" onClick={fetchJobDetail} disabled={!selectedJobId || !canUseProtectedApi}>
          선택 작업 상세 새로고침
        </button>
      </footer>
    </div>
